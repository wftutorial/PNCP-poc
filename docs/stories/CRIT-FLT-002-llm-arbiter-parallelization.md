...
- [ ] **AC1:** Paralelizar chamadas LLM da Camada 3A usando `ThreadPoolExecutor(max_workers=10)` ou `asyncio.gather` (igual ao zero-match)
- [ ] **AC2:** Manter QA audit sampling funcional dentro do paralelismo
- [ ] **AC3:** Preservar contadores de stats (`aprovadas_llm_arbiter`, `rejeitadas_llm_arbiter`, `llm_arbiter_calls`) com thread-safety
- [ ] **AC4:** Fallback on LLM failure = REJECT (manter filosofia zero-noise)
- [ ] **AC5:** Log consolidado de tempo total de Camada 3A (antes vs depois)
- [ ] **AC6:** Manter cache MD5 de LLM arbiter funcional (thread-safe por design, cache é dict read-heavy)
- [ ] **AC7:** Testes unitários validam paralelismo (mock LLM com sleep para simular latência)
...
- `backend/filter.py` (linhas ~2792-2900, Camada 3A)
- `backend/tests/test_filter.py` (testes de paralelismo)
