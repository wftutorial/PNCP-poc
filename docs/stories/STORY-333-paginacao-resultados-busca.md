# STORY-333: Paginação de resultados de busca + recuperação de botões Excel/PDF + fix histórico
...
### 3. Botão "Próximo" ilegível no histórico
...
- [ ] AC10: A barra de ações contém: `[Ordenar por ▼] [Baixar Excel] [Relatório PDF] [X de Y oportunidades]`
...
### Bloco 3: Fix botão histórico

- [ ] AC14: No `historico/page.tsx`, atualizar estilo dos botões Anterior/Próximo para usar o mesmo componente `Pagination` do AC1 (consistência visual)
- [ ] AC15: Se não usar componente compartilhado, pelo menos corrigir: `disabled:opacity-30` → `disabled:opacity-50` e adicionar `disabled:cursor-not-allowed`
- [ ] AC16: Aumentar tamanho do texto: `text-sm` → `text-base` e padding: `px-3 py-1` → `px-4 py-2`
- [ ] AC17: Adicionar `font-medium` para melhorar legibilidade
- [ ] AC18: Testar em tema Light e Dark — botões devem ser legíveis em ambos

### Bloco 4: Testes

- [ ] AC19: Teste de `Pagination` component: renderiza, navega, reseta, persiste pageSize
- [ ] AC20: Teste de `SearchResults` com paginação: 100 items → mostra 20, navega para página 2 → mostra items 21-40
- [ ] AC21: Teste que botões Excel/PDF estão visíveis no viewport sem scroll (dentro da sticky bar)
- [ ] AC22: Teste de acessibilidade: `Pagination` tem `aria-label`, `aria-current="page"`, botões disabled com `aria-disabled`
- [ ] AC23: Teste de histórico: botão "Próximo" é visível e legível (contrast ratio check)
...
- Excel/PDF geram arquivo com TODOS os resultados (não apenas a página atual)
